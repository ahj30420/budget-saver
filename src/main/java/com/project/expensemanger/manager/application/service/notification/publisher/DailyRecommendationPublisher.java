...
public class DailyRecommendationPublisher {
    ...
    public void sendDailyRecommendation() {
        ...
        users.forEach(user -> {
           var recomendation = consultingService.getRecommendation(user.getId());
           eventPublisher.publishEvent(new DailyRecommendationEvent(recomendation, user.getId()));
        });
    }
}
